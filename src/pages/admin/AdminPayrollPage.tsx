...
const AdminPayrollPage = () => {
  ...
  const { payslips, loading } = useAppSelector((state: any) => state.payroll);
  const { employees } = useAppSelector((state: any) => state.employee);
  const { selectedCountry, countries } = useAppSelector((state: any) => state.settings);
  ...
  // Auto calculated deductions preview
  ...
  // Update form when selected country changes
  useEffect(() => {
    if (selectedCountry) {
      setForm((prev) => ({
        ...prev,
  ...
  // Auto calculate preview when form values change
  useEffect(() => {
    ...
    if (currentPolicy) {
      ...
      if (currentPolicy.hasAgeBased) {
        ...
        // Social contribution
        socialContribution = Math.round(basic * currentPolicy.socialContributionEmployeeRate / 100 * 100) / 100;
      }
    }
        ...
        <div className="rounded-2xl p-4 mb-4 text-sm border bg-blue-50 border-blue-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="font-bold text-gray-700">
              {selectedCountry.countryName} Payroll Policy
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-gray-600">
            ...
            {selectedCountry.hasProgressiveTax && (
              <p>🏛️ Tax: <strong>Progressive Income Tax</strong></p>
            )}
            {selectedCountry.hasAgeBased && (
              <p>👴 Contribution: <strong>Age-based rates</strong></p>
            )}
          </div>
        </div>
      )}
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setForm({
                            ...
                            country: c?.countryName || e.target.value,
                          });
                        }}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                      >
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm sticky top-0">
                    ...
                    {currentPolicy?.hasAgeBased && preview.cpfEmployer > 0 && (
                      <PreviewRow
                        label={`${currentPolicy.socialContributionLabel} Employer`}
                        value={preview.cpfEmployer}
                        currency={currency}
                      />
                    )}
