...
import { exportPayslipPDF } from '../../utils/exportPdf';
  ...
  return (
    <Layout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">View and download your salary history</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['Period', 'Country', 'Gross Salary', 'Total Deduction', 'Net Salary', 'Actions'].map(
                  (h) => (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-gray-500 hover:text-gray-700 text-xs font-semibold"
                      >
                        <Download size={12} />
                        PDF
                      </button>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-screen overflow-y-auto">
            ...
            <div className="bg-indigo-50 rounded-xl p-4 mb-4">
              ...
              <p className="text-xs text-indigo-600">
                {selected.country} · {getCurrency(selected.country)}
              </p>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="space-y-1.5">
                ...
                {selected.allowance > 0 && <Row label="Allowance" value={fmt(selected.allowance, selected.country)} />}
                {selected.overtimePay > 0 && <Row label="Overtime Pay" value={fmt(selected.overtimePay, selected.country)} />}
                {selected.yearEndBonus > 0 && <Row label="Year End Bonus" value={fmt(selected.yearEndBonus, selected.country)} />}
                {selected.thirteenthMonth > 0 && <Row label="13th Month" value={fmt(selected.thirteenthMonth, selected.country)} />}
            ...
            <div className="mb-4">
              ...
              <div className="space-y-1.5">
                {selected.country === 'Myanmar' && (
                  <>
                    {selected.taxDeduction > 0 && <Row label="Income Tax (Progressive)" value={fmt(selected.taxDeduction, selected.country)} negative />}
                    {selected.socialSecurity > 0 && <Row label="SSB Contribution (2%)" value={fmt(selected.socialSecurity, selected.country)} negative />}
                  </>
                )}
                {selected.country === 'Singapore' && (
                  <div className="bg-blue-50 rounded-lg p-3 mb-2">
                    <p className="text-xs font-bold text-blue-600 mb-1">CPF Reference (Paid directly to IRAS)</p>
                    {selected.cpfEmployee > 0 && <Row label="CPF Employee" value={fmt(selected.cpfEmployee, selected.country)} blue />}
                    {selected.cpfEmployer > 0 && <Row label="CPF Employer" value={fmt(selected.cpfEmployer, selected.country)} blue />}
                  </div>
                )}
                {selected.country === 'Thailand' && (
                  <>
                    {selected.socialSecurity > 0 && <Row label="SSF Contribution (5%)" value={fmt(selected.socialSecurity, selected.country)} negative />}
                    {selected.taxDeduction > 0 && <Row label="Income Tax" value={fmt(selected.taxDeduction, selected.country)} negative />}
                  </>
                )}
                {selected.country === 'Malaysia' && (
                  <>
                    {selected.socialSecurity > 0 && <Row label="EPF Employee (11%)" value={fmt(selected.socialSecurity, selected.country)} negative />}
                    {selected.taxDeduction > 0 && <Row label="Income Tax" value={fmt(selected.taxDeduction, selected.country)} negative />}
                  </>
                )}
                {selected.country === 'Philippines' && (
                  <>
                    {selected.socialSecurity > 0 && <Row label="SSS Contribution (4.5%)" value={fmt(selected.socialSecurity, selected.country)} negative />}
                    {selected.taxDeduction > 0 && <Row label="Income Tax" value={fmt(selected.taxDeduction, selected.country)} negative />}
                  </>
                )}
                {selected.loanDeduction > 0 && <Row label="Loan Deduction" value={fmt(selected.loanDeduction, selected.country)} negative />}
                {selected.otherDeduction > 0 && <Row label="Other Deduction" value={fmt(selected.otherDeduction, selected.country)} negative />}
                <div className="border-t border-gray-100 pt-1.5">
            ...
            {selected.notes && (
              <p className="text-xs text-gray-400 mb-4">Notes: {selected.notes}</p>
            )}
