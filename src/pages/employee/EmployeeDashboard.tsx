    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl shadow-sm p-6 flex items-center gap-4">
          <div className="bg-indigo-500 text-white text-2xl w-12 h-12 rounded-xl flex items-center justify-center">📋</div>
          <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-2xl font-bold text-gray-800">{recentPayslips.length}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-indigo-50 rounded-xl p-4">
              <p className="text-sm font-semibold text-indigo-700 mb-3">Annual Leave</p>
              ...
              {balance.carryForward > 0 && (
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-semibold text-green-600">+{balance.carryForward} days</span>
                </div>
              )}
              <div className="flex justify-between text-sm mb-2">
                <span className="text-gray-500">Used</span>
                <span className="font-semibold text-red-500">-{balance.annualUsed} days</span>
              </div>
              <div className="border-t border-indigo-100 pt-2 flex justify-between text-sm">
                ...
                <span className="font-bold text-indigo-700">{balance.annualRemaining} days</span>
              </div>
              <div className="mt-3 bg-indigo-100 rounded-full h-2">
                <div
                  className="bg-indigo-600 h-2 rounded-full"
                  style={{ width: `${Math.min((balance.annualUsed / balance.totalAnnualAvailable) * 100, 100)}%` }}
                />
              </div>
            </div>
            <div className="bg-green-50 rounded-xl p-4">
              <p className="text-sm font-semibold text-green-700 mb-3">Medical Leave</p>
              <div className="flex justify-between text-sm mb-2">
              ...
              <div className="flex justify-between text-sm mb-2">
                <span className="text-gray-500">Used</span>
                <span className="font-semibold text-red-500">-{balance.medicalUsed} days</span>
              </div>
              <div className="border-t border-green-100 pt-2 flex justify-between text-sm">
                ...
                <span className="font-bold text-green-700">{balance.medicalRemaining} days</span>
              </div>
              <div className="mt-3 bg-green-100 rounded-full h-2">
                <div
                  className="bg-green-600 h-2 rounded-full"
                  style={{ width: `${Math.min((balance.medicalUsed / balance.medicalTotal) * 100, 100)}%` }}
                />
              </div>
            </div>
          </div>
        ...
        {recentPayslips.length === 0 ? (
          ...
          <div className="space-y-3">
            {recentPayslips.map((p) => (
              <div key={p.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition">
                <div>
